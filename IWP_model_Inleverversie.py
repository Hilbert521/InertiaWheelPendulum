...
step_start_states 	= False #used for sweeping through start states

control             = True #PID controllers
energy_control      = True #Swingup controllers
impulse_on          = False #Impulse at t=0 to analyize system characteristics
max_u               = 0.30 #max controller action (saturation estimated using a bump test but more thorough testing is neded)
...
for file_num in range(num_of_simulations):
  ...
  for T in linspace(0,max_time,max_time/(dt)):
      ...
      #Discretize the controller action to update at 10 Hz
      if int(T*1000)%10 != 0:
          u = u_old
